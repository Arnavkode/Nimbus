...
import { User, Lock, ShieldCheck, Loader2, LogIn, Info, X, AlertTriangle } from 'lucide-react';
...
const InputField = ({ id, label, type = 'text', placeholder = '', value, onChange, icon }) => {
    ...
    <div className="mb-3 sm:mb-4">
      <label htmlFor={id} className="block text-xs sm:text-sm text-gray-300 mb-1 sm:mb-2">{label}</label>
      <div className="relative">
        <span className="absolute inset-y-0 left-0 flex items-center pl-2 sm:pl-3">{React.cloneElement(icon, { size: 16 })}</span>
        <input
          id={id}
          name={id}
          type={type}
    ...
    <div className="relative min-h-screen w-full p-2 sm:p-4 animated-gradient-bg">
            ...
            <div className="bg-black/40 backdrop-blur-xl rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 border-2 border-white/20">
              ...
              <div className="text-center mb-4 sm:mb-6 md:mb-8">
                <div className="inline-flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-yellow-500/20 to-orange-500/20 mb-3 sm:mb-4">
                  <AlertTriangle size={24} className="sm:w-10 sm:h-10 md:w-12 md:h-12 text-yellow-400" />
                </div>
                <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-2 sm:mb-3">INSTRUCTIONS BEFORE USE</h2>
              </div>
              ...
              <div className="space-y-3 sm:space-y-4 md:space-y-6 mb-6 sm:mb-8">
                <div className="flex items-start gap-3 sm:gap-4 p-3 sm:p-4 rounded-lg bg-white/5 border border-white/10">
                  <div className="flex-shrink-0 w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-cyan-500/20 flex items-center justify-center mt-0.5">
                    <span className="text-sm sm:text-lg md:text-xl font-bold text-cyan-400">1</span>
                  </div>
                  <p className="text-sm sm:text-base md:text-lg text-gray-300 flex-1 pt-0.5">
                    Please do not use on <span className="font-semibold text-yellow-400">Thapar internet</span> as Fortinet does not allow API calls on an ngrok tunnel
                  </p>
                </div>

                <div className="flex items-start gap-3 sm:gap-4 p-3 sm:p-4 rounded-lg bg-white/5 border border-white/10">
                  <div className="flex-shrink-0 w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-cyan-500/20 flex items-center justify-center mt-0.5">
                    <span className="text-sm sm:text-lg md:text-xl font-bold text-cyan-400">2</span>
                  </div>
                  <p className="text-sm sm:text-base md:text-lg text-gray-300 flex-1 pt-0.5">
                    If the server is not running please call <span className="font-semibold text-cyan-400">Arnav</span> once, to turn on the machine
                  </p>
                </div>
              </div>
      ...
      <div className="flex items-center justify-center min-h-screen relative z-10 py-4 sm:py-0">
        <div className="w-full sm:w-3/4 max-w-4xl rounded-xl sm:rounded-2xl shadow-2xl animated-gradient-bg p-[2px] mx-auto">
          <main className="w-full p-4 sm:p-8 md:p-12 bg-black/20 backdrop-blur-lg rounded-[18px] sm:rounded-[22px] noise-overlay"
            ...
            <div className="text-center mb-4 sm:mb-8 relative z-10">
              <div className="flex justify-center items-center mb-2 sm:mb-4">
                <NimbusVaultLogo />
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white ml-2">NimbusVault</h1>
              </div>
              <p className="text-sm sm:text-base text-gray-400">Unleashing the Power of Your Data</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="relative z-10">
                  ...
                  {error === 'Failed to connect to the server.' && (
                    <div className="mt-3 pt-3 border-t border-red-500/30 space-y-2">
                      <div className="flex items-start gap-2">
                        ...
                        <p className="text-xs sm:text-sm">Please do not use on <span className="font-semibold">Thapar internet</span> as Fortinet does not allow API calls on an ngrok tunnel</p>
                      </div>
                      <div className="flex items-start gap-2">
                        <span className="text-red-400 font-bold flex-shrink-0">2.</span>
                        <p className="text-xs sm:text-sm">If the server is not running please call <span className="font-semibold">Arnav</span> once, to turn on the machine</p>
                      </div>
                    </div>
                  )}
                </div>
              )}
              {success && <div className="mb-3 sm:mb-4 p-2 sm:p-3 text-xs sm:text-sm bg-green-500/20 text-green-300 rounded-lg border border-green-500/30">{success}</div>}
              ...
              <div className="mt-4 sm:mt-6">
                <button type="submit"
                  ...
                  disabled={isLoading}>
                  {isLoading ? <Loader2 size={18} className="animate-spin" /> : (isLoginMode ? <LogIn size={18} /> : <ShieldCheck size={18} />)}
